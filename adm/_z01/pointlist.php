...
$ser_type = isset($_GET['ser_type']) ? clean_xss_tags($_GET['ser_type']) : '';
...
// 포인트 유형 필터
if (!empty($ser_type)) {
    $ser_type = addslashes($ser_type);
    $where[] = " pt.type = '{$ser_type}' ";
}
...
if ($stx) {
    switch ($_GET['sfl']) {
        ...
        case 'payment_id':
            ...
            break;
        case 'shop_id':
            $where[] = " pt.shop_id = '".addslashes($stx)."' ";
            break;
        case 'shop_name':
...
<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    <div class="mb-2 sch_last">
        ...
        <select name="sfl" id="sfl" class="frm_input">
            ...
            <option value="shop_id"<?php echo $_GET['sfl'] == 'shop_id' ? ' selected' : '' ?>>가맹점ID</option>
            <option value="shop_name"<?php echo $_GET['sfl'] == 'shop_name' ? ' selected' : '' ?>>가맹점명</option>
        </select>
        ...
        <table class="table table-bordered table-condensed tbl_sticky_100">
            ...
            <thead>
                <tr class="success">
                    ...
                    <th scope="col" class="td_left">가맹점</th>
            ...
            <tbody>
                ...
                foreach ($earn_points as $earn_id => $earn_row) {
                    ...
                    // 삭제 가능 여부 확인 (옵션 1: 관련 거래가 없을 때만 삭제 가능)
                    $can_delete_earn = empty($earn_row['related_transactions']) || count($earn_row['related_transactions']) == 0;
                ?>
                <!-- 적립 포인트 (메인 행) -->
                <tr class="<?=$bg?>" style="background-color: #f0f8ff;" tr_id="<?=$point_id?>">
                    <td class="td_chk">
                    ...
                    <td class="td_center"><strong><?=$type_arr['적립']?></strong></td>
                    <td class="td_num"><strong><?=$amount_display?></strong></td>
                    <td class="td_left">-</td>
                    <td class="td_left font_size_8"><?=$earn_row['payment_id']?></td>
                    <td class="td_left">
                        <?php if ($earn_row['shop_id']) { ?>
                            <a href="?sfl=shop_id&stx=<?=$earn_row['shop_id']?>"><?=get_text($earn_row['shop_name'])?:('ID: '.$earn_row['shop_id'])?></a>
                        <?php } else { ?>
                            -
                        <?php } ?>
                    </td>
                    ...
                    <td class="td_mngsmall">
                        ...
                        <?php } else { ?>
                            <span style="color:#999; cursor:not-allowed;" title="관련 거래가 있어 삭제할 수 없습니다.">삭제불가</span>
                        <?php } ?>
                    </td>
                </tr>
                ...
                <tr class="<?=$related_bg?>" style="background-color: #fafafa;" tr_id="<?=$related_point_id?>">
                    ...
                    <td class="td_left">
                        <?php if ($related_row['shop_id']) { ?>
                            <a href="?sfl=shop_id&stx=<?=$related_row['shop_id']?>"><?=get_text($related_row['shop_name'])?:('ID: '.$related_row['shop_id'])?></a>
                        <?php } else { ?>
                            -
                        <?php } ?>
                    </td>
