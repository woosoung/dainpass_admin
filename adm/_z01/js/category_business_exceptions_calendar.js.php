...
function changeMonth(delta) {
    ...
    location.href = './category_business_exceptions_calendar.php?sca=' + sca + '&year=' + year + '&month=' + month;
}
...
function addException() {
    try {
        ...
        // 현재 선택된 category_id 설정
        var currentCategoryId = document.getElementById('category-id').value;
        if (currentCategoryId && currentCategoryId !== 'all') {
            document.getElementById('modal_category_id_select').value = currentCategoryId;
        } else {
...
// 예외일 수정
function editException(category_id, date) {
    // AJAX로 해당 날짜의 데이터 가져오기
    $.ajax({
        url: './ajax/category_business_exceptions_get.php',
        type: 'POST',
        data: {
        ...
        success: function(response) {
            if (response.success && response.data) {
                ...
                document.getElementById('modal_reason').value = data.reason || '';
...
// 예외일 삭제
function deleteException(event, category_id, date) {
    event.stopPropagation();
    
    if (!confirm('해당 특별휴무/영업일시를 삭제하시겠습니까?')) {
        return false;
    }
    
    $.ajax({
        url: './ajax/category_business_exceptions_del.php',
        type: 'POST',
        data: {
            category_id: category_id,
            date: date
        ...
        success: function(response) {
            if (response.success) {
                // 해당 라벨 삭제
                $('[data-category-id="' + category_id + '"][data-date="' + date + '"].exception-label').fadeOut(300, function() {
                    $(this).remove();
                });
                alert('삭제되었습니다.');
            } else {
...
function saveException() {
    ...
    // 디버깅: 전송할 데이터 확인
    console.log('Saving exception with data:', ajaxData);
    
    // AJAX로 저장
    $.ajax({
        url: './category_business_exceptions_list_update.php',
        type: 'POST',
        data: ajaxData,
        dataType: 'html',
        success: function(response) {
...
$(document).ready(function() {
    // 페이지 로드 완료
    console.log('Category Business Exceptions Calendar loaded');
});
</script>
