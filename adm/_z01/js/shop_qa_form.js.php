...
$(document).ready(function() {
    ...
    if (originalSecretYn == 'N') {
        $('input[name="inq_reply_secret_yn"]').on('change', function() {
            if ($(this).val() == 'Y') {
        ...
        // 초기 상태 확인
        if ($('input[name="inq_reply_secret_yn"]:checked').val() == 'Y') {
            $('#secret_warning').show();
        }
    }
});

function fshopqaform_submit(f)
{
    if (!f.inq_reply_content.value || f.inq_reply_content.value.trim() == '') {
        alert("답변 내용을 입력해주세요.");
        f.inq_reply_content.focus();
        return false;
    }
    
    // 본 문의글이 공개글인데 댓글에 비밀글 설정하는 경우 확인
    var originalSecretYn = document.getElementById('original_secret_yn').value;
    var replySecretYn = f.inq_reply_secret_yn.value;
