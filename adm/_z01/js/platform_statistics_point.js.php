...
(function($) {
    ...
    function loadPointStatistics() {
        ...
        $.ajax({
            ...
            renderShopEarnedRankChart(data.shop_earned_rank);
    ...
    function updateSummaryCards(summary) {
        ...
        $('#total_expired').text(formatPoint(summary.total_expired || 0));
        ...
        $('#expiry_rate').text(formatPercent(summary.expiry_rate || 0));
    }
    ...
    function renderTransactionTypeChart(distributionData) {
        ...
        var data = [
            ...
            distributionData['소멸'] || 0
        ];
        var colors = ['#4BC0C0', '#36A2EB', '#FF6384']; // 적립: 청록색, 사용: 파란색, 소멸: 빨간색
    ...
    function renderPointTrendChart(trendData) {
        ...
        for (var i = 0; i < trendData.length; i++) {
            ...
            expiredData.push(trendData[i].expired || 0);
        }
    ...
    // 가맹점별 포인트 적립 기여도 차트 (가로 막대 차트)
    function renderShopEarnedRankChart(rankData) {
        ...
        for (var i = 0; i < Math.min(rankData.length, 20); i++) {
            ...
            data.push(rankData[i].total_earned || 0);
        }

        var ctx = document.getElementById('shop_earned_rank_chart');
        if (!ctx) return;

        if (shopEarnedRankChart) {
            shopEarnedRankChart.destroy();
        }

        shopEarnedRankChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return '적립량: ' + formatPoint(context.parsed.x || 0);
                            }
                        }
                    }
                },
    ...
    function renderTransactionTable(transactionList) {
        ...
        for (var i = 0; i < transactionList.length; i++) {
            ...
            var amountSign = '';
            if (item.transaction_type === '적립') {
                amountSign = '+';
            } else if (item.transaction_type === '사용' || item.transaction_type === '소멸') {
                amountSign = '-';
