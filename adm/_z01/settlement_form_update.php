...
// 수정 처리
    ...
    if (!$settlement_id) {
        $first_settlement = sql_fetch_pg(" SELECT settlement_id, payment_id, appointment_datetime FROM shop_settlements WHERE ssl_id = '{$ssl_id}' ORDER BY settlement_id LIMIT 1 ");
        if ($first_settlement) {
            $settlement_id = $first_settlement['settlement_id'];
            // 기존 값 저장 (업데이트 시 사용)
            $existing_payment_id = $first_settlement['payment_id'] ?? null;
            $existing_appointment_datetime = $first_settlement['appointment_datetime'] ?? null;
        }
    }
...
if($w == 'u') {
    goto_url('./settlement_form.php?'.$qstr.'&w=u&ssl_id='.$ssl_id, false);
}
else {
