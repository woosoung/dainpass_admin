...
else if ($w == "u")
{
    ...
    for($i=0;$row2=sql_fetch_array_pg($rs->result);$i++) {
        ...
        $row2['down_del'] = ($is_s3file_yn) ? $row2['fle_name_orig'].'&nbsp;&nbsp;<a href="'.G5_Z_URL.'/lib/download.php?file_path='.$row2['fle_path'].'&file_name_orig='.$row2['fle_name_orig'].'">[파일다운로드]</a>&nbsp;&nbsp;'.substr($row2['fle_reg_dt'],0,19).'&nbsp;&nbsp;<label for="del_'.$row2['fle_idx'].'" style="position:relative;top:-3px;cursor:pointer;"><input type="checkbox" name="bng_img_'.$row2['fle_db_idx'].'_del['.$row2['fle_idx'].']" id="del_'.$row2['fle_idx'].'" value="1"> 삭제</label>'.PHP_EOL : ''.PHP_EOL;
...
<section id="anc_banner_group">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row"><label for="bng_device">디바이스</label><strong class="sound_only">필수</strong></th>
            <td>
                <select name="bng_device" id="bng_device" class="frm_input" required>
                    <option value="">선택</option>
                    <option value="PC" <?php echo ($bg['bng_device'] == 'PC') ? 'selected' : ''; ?>>PC</option>
                    <option value="MOBILE" <?php echo ($bg['bng_device'] == 'MOBILE') ? 'selected' : ''; ?>>MOBILE</option>
                </select>
            </td>
            <th scope="row"><label for="bng_name">배너그룹명</label><strong class="sound_only">필수</strong></th>
            <td><input type="text" name="bng_name" value="<?php echo $bg['bng_name']; ?>" id="bng_name" size="38" required class="required frm_input"></td>
        </tr>
        <tr>
        ...
        <tr>
            <th scope="row"><label for="banner_group_img">배너그룹 이미지</label></th>
            <td colspan="3">
                <?php echo help("배너 그룹의 위치를 파악할 수 있는 이미지를 관리합니다. (최대 1개까지 업로드 가능합니다.)"); ?>
                <div>
...
<section id="anc_banner_list">
    ...
    <div class="tbl_frm01 tbl_wrap" style="margin-bottom: 30px;">
        ...
        <div id="banner_preview_area" style="display: none; margin-bottom: 20px; padding: 15px; background-color: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
            ...
            <div id="banner_preview_content" style="display: flex; gap: 15px; align-items: flex-start;">
                ...
                <div id="banner_img_preview" style="display: none;">
                    <div style="margin-bottom: 5px; font-size: 12px; color: #666;">기존 이미지</div>
                    <img id="banner_img_thumb" src="" alt="배너 이미지" style="width: 120px; height: 80px; border: 1px solid #ddd; object-fit: cover;">
                </div>
                <!-- 유튜브 썸네일 -->
        ...
        <tr>
            <th scope="row"><label for="bnr_link">링크 주소</label></th>
            <td>
                <input type="text" name="bnr_link" id="bnr_link" size="50" class="frm_input" placeholder="https://example.com 또는 dainpass://shop/detail?id=123">
                <?php echo help("PC: 일반 URL, 모바일: DeepLink 형식"); ?>
            </td>
        ...
        <tr>
            <th scope="row"><label for="banner_img">배너 이미지</label></th>
            <td colspan="3">
        ...
        <table id="banner_list_table" class="table table-bordered table-condensed">
            ...
            <thead>
                <tr class="success">
                    ...
                    <th scope="col" style="width: 320px;">섬네일</th>
                    <th scope="col">배너명</th>
                    <th scope="col">링크</th>
                    <th scope="col">유튜브</th>
                    <th scope="col">상태</th>
            ...
            <tbody id="banner_list">
                ...
                while($banner_row = sql_fetch_array_pg($banner_result->result)){
                    ...
                    if(!empty($banner_img_res['fle_path'])){
                        ...
                        $thumbnail_html .= '<div style="position:relative;display:inline-block;vertical-align:top;margin-right:5px;"><img src="'.$thumb_url.'" alt="배너 이미지" style="width:'.$thumb_wd.'px;height:'.$thumb_ht.'px;border:1px solid #ddd;display:block;"><i class="fa fa-image" style="position:absolute;top:2px;right:2px;background-color:rgba(0,0,0,0.6);color:#fff;padding:2px 4px;border-radius:3px;font-size:10px;" title="이미지"></i></div>';
                        $has_image = true;
                    }
                    ...
                    if(!empty($banner_row['bnr_youtube'])){
                        ...
                        if(preg_match('/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([a-zA-Z0-9_-]+)/', $banner_row['bnr_youtube'], $matches)){
                            ...
                            $thumbnail_html .= '<div style="position:relative;display:inline-block;vertical-align:top;"><img src="'.$youtube_thumb.'" alt="유튜브 썸네일" style="width:'.$thumb_wd.'px;height:'.$thumb_ht.'px;border:1px solid #ddd;display:block;"><i class="fa fa-youtube" style="position:absolute;top:2px;right:2px;background-color:rgba(255,0,0,0.8);color:#fff;padding:2px 4px;border-radius:3px;font-size:10px;" title="유튜브"></i></div>';
                            $has_youtube = true;
                        }
                    }
                    
                    if(empty($thumbnail_html)){
                        $thumbnail_html = '<img src="'.G5_Z_URL.'/img/no_thumb.png" alt="no image" style="width:'.$thumb_wd.'px;height:'.$thumb_ht.'px;border:1px solid #ddd;">';
                    }
                ?>
                <tr data-bnr-id="<?=$banner_row['bnr_id']?>">
                    ...
                    <td class="td_thumb" style="white-space: nowrap;"><?=$thumbnail_html?></td>
                    <td class="td_name"><?=get_text($banner_row['bnr_name'])?></td>
                    <td class="td_link"><?=!empty($banner_row['bnr_link']) ? cut_str($banner_row['bnr_link'], 30) : '-'?></td>
                    <td class="td_youtube"><?=!empty($banner_row['bnr_youtube']) ? '있음' : '-'?></td>
                ...
                if ($banner_count == 0)
                    echo "<tr class=\"no-data\"><td colspan=\"9\" class=\"empty_table\">등록된 배너가 없습니다.</td></tr>";
                ?>
            </tbody>
        </table>
    </div>
