...
// 최초 질문 조회 및 검증
$sql = " SELECT * FROM shop_qna WHERE qna_id = '$qna_id' AND shop_id IS NULL AND qna_parent_id IS NULL ";
$qna = sql_fetch_pg($sql);

if (!isset($qna['qna_id']) || !$qna['qna_id']) {
    alert("문의자료가 없거나 플랫폼 문의가 아닙니다.");
}

if ($w == 'r') {
    // 답변 등록
    $qna_reply_content = isset($_POST['qna_reply_content']) ? trim($_POST['qna_reply_content']) : '';
    $qna_reply_secret_yn = isset($_POST['qna_reply_secret_yn']) ? trim($_POST['qna_reply_secret_yn']) : 'N';
    ...
    $sql = " INSERT INTO shop_qna 
             ...
             VALUES 
             ('{$qna_id}', NULL, NULL, '', '".addslashes($qna_reply_content)."', '".addslashes($qna_ip)."', '{$qna_reply_secret_yn}', '".addslashes($reply_mb_id)."', 'pending', NOW(), NOW()) ";
    
    sql_query_pg($sql);
    
    alert("답변이 등록되었습니다.", "./customer_qa_form.php?qna_id={$qna_id}");
}
else if ($w == 'u') {
    // 답변 수정
    ...
    // 답변 조회 및 검증
    $sql_reply = " SELECT * FROM shop_qna WHERE qna_id = '$reply_id' AND qna_parent_id = '$qna_id' AND reply_mb_id IS NOT NULL ";
    ...
    // 답변 수정
    $sql = " UPDATE shop_qna 
             SET qna_content = '".addslashes($qna_reply_content)."', 
                 qna_updated_at = NOW() 
    ...
    alert("답변이 수정되었습니다.", "./customer_qa_form.php?qna_id={$qna_id}");
}
else if ($w == 'd') {
    // 답변 삭제
    $reply_id = isset($_POST['reply_id']) ? (int)$_POST['reply_id'] : 0;
    ...
    alert("답변이 삭제되었습니다.", "./customer_qa_form.php?qna_id={$qna_id}");
}

goto_url('./customer_qa_list.php', false);
