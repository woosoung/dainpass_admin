...
$sql = " SELECT sa.*, 
                ...
                -- 할인 적용 여부 확인 (쿠폰 할인)
                (SELECT COUNT(*) FROM appointment_shop_detail asd 
                 WHERE asd.appointment_id = sa.appointment_id 
                   AND asd.coupon_amount > 0) as has_coupon_discount,
                -- 할인 적용 여부 확인 (이벤트 할인)
                (SELECT COUNT(*) FROM appointment_shop_detail asd2
                 INNER JOIN shop_appointment_details sad ON asd2.shopdetail_id = sad.shopdetail_id
                 WHERE asd2.appointment_id = sa.appointment_id 
                   AND (sad.discount_price > 0 OR (sad.price * sad.quantity - sad.net_amount > 0))) as has_event_discount,
                -- 포인트 사용 여부 확인 (point_transactions에서 type='사용'인 경우)
                (SELECT COUNT(*) FROM payments p
                 INNER JOIN point_transactions pt ON p.payment_id = pt.payment_id
                 WHERE p.appointment_id = sa.appointment_id 
                   AND pt.type = '사용'
                   AND pt.amount > 0) as has_point_usage
         {$sql_common}
         {$sql_search}
         {$sql_order}
