...
if (!$check_result || !is_object($check_result) || !isset($check_result->result)) {
    $response['message'] = '예외일을 찾을 수 없습니다.';
    echo json_encode($response);
    exit;
}
...
if (!$check_row) {
    $response['message'] = '접근 권한이 없는 예외일입니다.';
    echo json_encode($response);
    exit;
}

// 삭제 실행
$delete_sql = " DELETE FROM business_exceptions WHERE shop_id = {$shop_id} AND date = '{$date}' ";
