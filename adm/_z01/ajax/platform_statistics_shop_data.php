...
// 기간 계산 함수 (shop_statistics_sales_data.php 참조)
...
try {
    ...
    // 8. 지역별 가맹점 분포 (주소 정보 기반, addr1 또는 addr2 활용)
    $sql = "
        SELECT 
            CASE 
                WHEN s.addr1 IS NOT NULL AND s.addr1 != '' THEN s.addr1
                WHEN s.addr2 IS NOT NULL AND s.addr2 != '' THEN s.addr2
                ELSE '미지정'
            END as region,
            COUNT(*) as shop_count
        FROM {$g5['shop_table']} s
    ...
    if ($result && is_object($result) && isset($result->result)) {
        while ($row = sql_fetch_array_pg($result->result)) {
            ...
            $status_kr = '미지정';
            switch ($row['status']) {
                case 'active': $status_kr = '정상'; break;
                case 'pending': $status_kr = '대기'; break;
                case 'closed': $status_kr = '폐업'; break;
                case 'shutdown': $status_kr = '금지'; break;
            }
