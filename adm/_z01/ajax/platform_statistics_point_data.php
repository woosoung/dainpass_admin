...
try {
    ...
    // 1-3. 총 포인트 소멸량 (누적)
    $sql = "SELECT COALESCE(SUM(ABS(amount)), 0) as total_expired
            FROM point_transactions
            WHERE type = '소멸'";
    $row = sql_fetch_pg($sql);
    $total_expired = (int)($row['total_expired'] ?? 0);
    
    // 1-4. 현재 플랫폼 전체 포인트 잔액
    $sql = "WITH point_summary AS (
                SELECT customer_id,
                       SUM(CASE WHEN type = '적립' THEN amount ELSE 0 END) as earned,
                       SUM(CASE WHEN type = '사용' THEN ABS(amount) ELSE 0 END) as used,
                       SUM(CASE WHEN type = '소멸' THEN ABS(amount) ELSE 0 END) as expired
                FROM point_transactions
                GROUP BY customer_id
            )
            SELECT COALESCE(SUM(earned - used - expired), 0) as total_balance
            FROM point_summary
            WHERE (earned - used - expired) > 0";
    $row = sql_fetch_pg($sql);
    ...
    $sql = "WITH point_summary AS (
                SELECT customer_id,
                       SUM(CASE WHEN type = '적립' THEN amount ELSE 0 END) - 
                       SUM(CASE WHEN type = '사용' THEN ABS(amount) ELSE 0 END) - 
                       SUM(CASE WHEN type = '소멸' THEN ABS(amount) ELSE 0 END) as balance
                FROM point_transactions
                GROUP BY customer_id
                HAVING SUM(CASE WHEN type = '적립' THEN amount ELSE 0 END) - 
                       SUM(CASE WHEN type = '사용' THEN ABS(amount) ELSE 0 END) - 
                       SUM(CASE WHEN type = '소멸' THEN ABS(amount) ELSE 0 END) > 0
            )
    ...
    $usage_rate = 0.0;
    if ($total_earned > 0) {
        $usage_rate = round(($total_used / $total_earned) * 100, 1);
    }
    
    // 1-8. 포인트 소멸률 (소멸량 / 적립량)
    $expiry_rate = 0.0;
    if ($total_earned > 0) {
        $expiry_rate = round(($total_expired / $total_earned) * 100, 1);
    }
    
    // 2. 차트 데이터
    
    // 2-1. 기간별 포인트 적립/사용/소멸 추이 (선 그래프)
    if ($period_type == 'weekly') {
        $date_group_expr = "DATE_TRUNC('week', COALESCE(earned_at, used_at, expired_at, created_at))::DATE";
        $date_series_expr = "DATE_TRUNC('week', date_series.date)::DATE";
        $date_series_filter = "AND EXTRACT(DOW FROM date_series.date) = 1";
    } elseif ($period_type == 'monthly') {
        $date_group_expr = "DATE_TRUNC('month', COALESCE(earned_at, used_at, expired_at, created_at))::DATE";
        $date_series_expr = "DATE_TRUNC('month', date_series.date)::DATE";
        $date_series_filter = "AND EXTRACT(DAY FROM date_series.date) = 1";
    } else {
        $date_group_expr = "DATE(COALESCE(earned_at, used_at, expired_at, created_at))";
    ...
    $sql = "WITH date_series AS (
            ...
            earned_data AS (
                ...
                FROM point_transactions
                WHERE type = '적립'
                  AND DATE(COALESCE(earned_at, created_at)) BETWEEN '{$range_start}' AND '{$range_end}'
            ...
            used_data AS (
                ...
                FROM point_transactions
                WHERE type = '사용'
                  AND DATE(COALESCE(used_at, created_at)) BETWEEN '{$range_start}' AND '{$range_end}'
                GROUP BY {$date_group_expr}
            ),
            expired_data AS (
                SELECT {$date_group_expr} as date,
                       SUM(ABS(amount)) as expired_amount
                FROM point_transactions
                WHERE type = '소멸'
                  AND DATE(COALESCE(expired_at, created_at)) BETWEEN '{$range_start}' AND '{$range_end}'
                GROUP BY {$date_group_expr}
            )
            SELECT 
    ...
    $sql = "SELECT 
                type,
                SUM(CASE WHEN type = '적립' THEN amount ELSE ABS(amount) END) as total_amount
            FROM point_transactions
            GROUP BY type";
    $result = sql_query_pg($sql);
    ...
    $sql = "SELECT 
                ...
                SUM(CASE WHEN type = '적립' THEN amount ELSE 0 END) - 
                SUM(CASE WHEN type = '사용' THEN ABS(amount) ELSE 0 END) - 
                SUM(CASE WHEN type = '소멸' THEN ABS(amount) ELSE 0 END) as net_increase
            FROM point_transactions
    ...
    $sql = "SELECT 
                ...
                (SELECT SUM(CASE WHEN pt2.type = '적립' THEN pt2.amount 
                                 WHEN pt2.type IN ('사용', '소멸') THEN -ABS(pt2.amount) 
                                 ELSE 0 END)
                 FROM point_transactions pt2
                 WHERE pt2.customer_id = pt.customer_id
                   AND (pt2.created_at < pt.created_at OR (pt2.created_at = pt.created_at AND pt2.point_id <= pt.point_id))
                ) as balance_after
            FROM point_transactions pt
            LEFT JOIN customers c ON pt.customer_id = c.customer_id
            LEFT JOIN shop s ON pt.shop_id = s.shop_id
    ...
    echo json_encode([
        ...
        'data' => [
            ...
            'shop_earned_rank' => $shop_earned_rank,
