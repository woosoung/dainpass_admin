...
try {
    ...
    // 총 쿠폰 발급 건수
    $sql = " SELECT COUNT(*) as total_issued
             FROM customer_coupons cc
             INNER JOIN shop_coupons sc ON cc.coupon_id = sc.coupon_id
             INNER JOIN shop s ON sc.shop_id = s.shop_id
             WHERE cc.issued_at::date BETWEEN '{$range_start}' AND '{$range_end}'
               {$category_filter_sql} {$region_filter_sql} ";
    $row = sql_fetch_pg($sql);
    $total_issued_count = (int)($row['total_issued'] ?? 0);
    
    // 총 쿠폰 사용 건수
    $sql = " SELECT COUNT(*) as total_used
             FROM customer_coupons cc
             INNER JOIN shop_coupons sc ON cc.coupon_id = sc.coupon_id
             INNER JOIN shop s ON sc.shop_id = s.shop_id
             WHERE cc.used_at::date BETWEEN '{$range_start}' AND '{$range_end}'
               AND (cc.status = 'USED' OR cc.used_at IS NOT NULL)
               {$category_filter_sql} {$region_filter_sql} ";
    $row = sql_fetch_pg($sql);
    $total_used_count = (int)($row['total_used'] ?? 0);
    ...
    $sql = " SELECT COALESCE(SUM(asd.coupon_amount), 0) as total_discount_amount
             ...
             WHERE asd.coupon_amount IS NOT NULL
               ...
               {$category_filter_sql} {$region_filter_sql} ";
    $row = sql_fetch_pg($sql);
    $total_discount_amount = (int)($row['total_discount_amount'] ?? 0);
    ...
    $sql = " SELECT COALESCE(AVG(asd.balance_amount), 0) as avg_order_amount
             ...
             WHERE asd.coupon_amount IS NOT NULL
               ...
               {$category_filter_sql} {$region_filter_sql} ";
    $row = sql_fetch_pg($sql);
    $avg_order_amount_with_coupon = (int)($row['avg_order_amount'] ?? 0);
    ...
    $sql = " SELECT COALESCE(SUM(asd.balance_amount - asd.coupon_amount), 0) as estimated_sales_increase
             ...
             WHERE asd.coupon_amount IS NOT NULL
               ...
               {$category_filter_sql} {$region_filter_sql} ";
    $row = sql_fetch_pg($sql);
    $estimated_sales_increase = (int)($row['estimated_sales_increase'] ?? 0);
    ...
    $sql = "
        ...
        discount_data AS (
            SELECT 
                {$date_group_expr} AS period_date,
                COALESCE(SUM(asd.coupon_amount), 0) AS discount_amount
            FROM appointment_shop_detail asd
            INNER JOIN shop s ON asd.shop_id = s.shop_id
            WHERE asd.coupon_amount IS NOT NULL
              AND asd.coupon_amount > 0
    ...
    $sql = "
        SELECT 
            ...
            COUNT(DISTINCT cc.customer_coupon_id) FILTER (WHERE cc.issued_at::date BETWEEN '{$range_start}' AND '{$range_end}') as issued_count,
            COUNT(DISTINCT cc.customer_coupon_id) FILTER (WHERE cc.used_at::date BETWEEN '{$range_start}' AND '{$range_end}' AND (cc.status = 'USED' OR cc.used_at IS NOT NULL)) as used_count
        FROM shop s
        INNER JOIN shop_coupons sc ON s.shop_id = sc.shop_id
        LEFT JOIN customer_coupons cc ON sc.coupon_id = cc.coupon_id
          AND (cc.issued_at::date BETWEEN '{$range_start}' AND '{$range_end}' OR cc.used_at::date BETWEEN '{$range_start}' AND '{$range_end}')
    ...
    $sql = "
        ...
        SELECT 
            ...
            END as usage_rate
        FROM date_periods dp
        LEFT JOIN issued_daily i ON dp.period_date = i.date
        LEFT JOIN used_daily u ON dp.period_date = u.date
        ORDER BY dp.period_date ASC
    ";
    $result = sql_query_pg($sql);
    ...
    if ($result && is_object($result) && isset($result->result)) {
        while ($row = sql_fetch_array_pg($result->result)) {
            $coupon_sales_contribution[] = [
                'shop_id' => (int)$row['shop_id'],
                'shop_name' => $row['shop_name'],
                'coupon_order_count' => (int)$row['coupon_order_count'],
                'total_sales_with_coupon' => (int)$row['total_sales_with_coupon'],
    ...
    $sql = "
        SELECT 
            ...
            COUNT(DISTINCT cc.customer_coupon_id) FILTER (WHERE cc.used_at::date BETWEEN '{$range_start}' AND '{$range_end}' AND (cc.status = 'USED' OR cc.used_at IS NOT NULL)) as used_count,
            CASE 
                WHEN COUNT(DISTINCT cc.customer_coupon_id) FILTER (WHERE cc.issued_at::date BETWEEN '{$range_start}' AND '{$range_end}') > 0 
                THEN (COUNT(DISTINCT cc.customer_coupon_id) FILTER (WHERE cc.used_at::date BETWEEN '{$range_start}' AND '{$range_end}' AND (cc.status = 'USED' OR cc.used_at IS NOT NULL))::float / COUNT(DISTINCT cc.customer_coupon_id) FILTER (WHERE cc.issued_at::date BETWEEN '{$range_start}' AND '{$range_end}')::float * 100)
                ELSE 0 
            END as usage_rate,
            COALESCE(SUM(asd.coupon_amount), 0) as total_discount,
        ...
        LEFT JOIN appointment_shop_detail asd ON asd.customer_coupon_id = cc.customer_coupon_id
          ...
          AND asd.status != 'BOOKED'
        WHERE 1=1 {$category_filter_sql} {$region_filter_sql}
        GROUP BY s.shop_id, s.shop_name, s.name, sc.name
