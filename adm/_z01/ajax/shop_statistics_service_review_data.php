...
if (!defined('SHOP_STAT_LIB_MODE')) {
    try {
        ...
        if (isset($_SESSION['ss_shop_id']) && $_SESSION['ss_shop_id']) {
            $shop_id = (int)$_SESSION['ss_shop_id'];
        } elseif (isset($_POST['shop_id']) && $_POST['shop_id']) {
            $shop_id = (int)$_POST['shop_id'];
        } else {
            // check_shop_access 함수 사용
        ...
        if (!$shop_id) {
            echo json_encode(['success' => false, 'message' => '가맹점 정보를 찾을 수 없습니다.'], JSON_UNESCAPED_UNICODE);
            exit;
        }
